...
function Step07-Shortcuts {
    ...
    $vbsBody = (
        ...
        "' Auto-generated by install.ps1. UTF-8-BOM-safe: keep this file GBK/ANSI if edited.`r`n" +
        "Set sh = CreateObject(`"WScript.Shell`")`r`n" +
        # VBScript 内嵌引号必须双写转义：""C:\..."" 才不会截断字符串
        "sh.Run `"powershell -NoProfile -ExecutionPolicy Bypass -File $q$q{0}$q$q`", 0, False`r`n" -f $scriptPs1
    )
    if ($script:DryRun) {
